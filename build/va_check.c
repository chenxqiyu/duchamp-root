...
_Static_assert(DIRECT_MAP_BASE == 0xffffff8000000000ULL, "linear region start");
_Static_assert(DIRECT_MAP_END == 0xffffffc000000000ULL, "linear region end (39-bit)");
_Static_assert(KERNELSNITCH_IDENTITY_START == P0_PAGE_OFFSET + P0_PHYS_OFFSET,
               "ksnitch window starts at DRAM linear alias");
_Static_assert(KERNELSNITCH_IDENTITY_END - KERNELSNITCH_IDENTITY_START == 16ULL << 30,
               "ksnitch window covers exactly 16GB DRAM");
